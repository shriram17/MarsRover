...
//
// Purpose: Handles all the teleport logic (extended)
//
//=============================================================================
...
{
	//-------------------------------------------------------------------------
	public class TeleportGeneric : MonoBehaviour
	{
        ...
        public Transform validReticleTransform;
        ...
        public bool allowGeneralTeleportation;
        ...
        public TeleportMarkerBase pointedAtTeleportMarkerForGeneralTeleportation;
        private TeleportMarkerBase teleportingToMarker;
        private Vector3 pointedAtGeneralTeleportationPoint;
        private bool pointedAtGeneralTeleportMarkerIsValid = false;
		...
		//-------------------------------------------------
		private static TeleportGeneric _instance;
		public static TeleportGeneric instance
		{
			get
			{
				if ( _instance == null )
				{
					_instance = GameObject.FindObjectOfType<TeleportGeneric>();
				}
		...
		void Update()
		{
            ...
            validReticleTransform.gameObject.SetActive(false);
		...
		private void UpdatePointer()
		{
            ...
            bool reticleAtBadAngle;

            if (hitSomething)
            {
                pointerEnd = hitInfo.point;
                reticleAtBadAngle = (Vector3.Dot(hitInfo.normal, Vector3.up) <= 0.8f);
            }
            else
            {
                pointerEnd = teleportArc.GetArcPositionAtTime(teleportArc.arcDuration);
                reticleAtBadAngle = true;
            }
            ...
            if ( hitTeleportMarker != null ) //Hit a teleport marker
			{
                ...
                pointedAtGeneralTeleportMarkerIsValid = false;
                pointerEnd = hitInfo.point;
			}
            else if (allowGeneralTeleportation && !pointerAtBadAngle && !reticleAtBadAngle)
            {
                destinationReticleTransform.gameObject.SetActive(false);
                offsetReticleTransform.gameObject.SetActive(false);
                validReticleTransform.gameObject.SetActive(true);
                ...
                validReticleTransform.rotation = Quaternion.Slerp(validReticleTransform.rotation, invalidReticleTargetRotation, 0.1f);

                //Scale the valid reticle based on the distance from the player
                ...
                validReticleTransform.transform.localScale = invalidReticleScale;

                pointedAtTeleportMarkerForGeneralTeleportation.transform.position = validReticleTransform.position;
                pointedAtGeneralTeleportMarkerIsValid = true;
			...
			else // Hit neither
			{
                ...
                pointedAtGeneralTeleportationPoint = new Vector3(0.0f, 0.0f, 0.0f);
                pointedAtGeneralTeleportMarkerIsValid = false;
                pointedAtTeleportMarker = null;
            ...
            validReticleTransform.position = pointerEnd;
		...
		{
			if (( pointerHand.controller != null ) && (!allowGeneralTeleportation))
			{
				if ( validLocation )
				{
		...
		private void TryTeleportPlayer()
		{
			if ( visible && !teleporting )
			{
                ...
                else if (allowGeneralTeleportation && pointedAtGeneralTeleportMarkerIsValid)
                {
                    //Pointing at an unlocked teleport marker
                    teleportingToMarker = pointedAtTeleportMarkerForGeneralTeleportation;
                    InitiateTeleportFade();
